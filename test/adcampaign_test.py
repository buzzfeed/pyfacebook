...
class TestAdCampaignApi():
  ...
  def test_find_by_adaccount_id(self):
    ...
    # Check order
    first_ten_adcampaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID, limit=10, offset=0 )
    second_five_adcampaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID, limit=5, offset=5 )
    ...
    all_campaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID )
    total = len(all_campaigns)
    limit = 3
    offset = total - limit + 1
    if offset < 0:  # facebook api does not support negative offsets
            offset = 0
    last_batch_of_campaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID, limit=limit, offset=offset )
    eq_( len(last_batch_of_campaigns), total - offset )

    # Check empty results
    no_campaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID, offset=total )
    eq_( no_campaigns, [] )

    # Check full results
    all_campaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID, offset=0, limit=total+1000)
    eq_( len(all_campaigns), total )

  def test_find_by_adgroup_id_and_find_by_id(self):
    adgroups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID, limit=2)

    adgroup = adgroups[0]

    adcampaign = self.fb.api().adcampaign().find_by_adgroup_id(adgroup.id)
    adcampaign_by_id = self.fb.api().adcampaign().find_by_id(adgroup.campaign_id)
  ...
  def test_find_by_ids( self ):
    adcampaign_api = Facade(self.fb.api().adcampaign())
    base_adcampaigns = adcampaign_api.find_by_adaccount_id( FACEBOOK_PROD_ACCOUNT_ID, limit=25 )
    ...
    adcampaigns = adcampaign_api.find_by_ids( test_adcampaign_ids )
    ...
    try:
        adcampaigns = adcampaign_api.find_by_ids([])
    except Exception, e:
