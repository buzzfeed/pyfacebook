...
class TestAdGroupApi():
    ...
    def test_find_by_adaccount_id(self):
        # Check order
        first_ten_adgroups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID, limit=10, offset=0)
        second_five_adgroups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID, limit=5, offset=5)
        ...
        all_groups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID)
        total = len(all_groups)

        limit = 3
        offset = total - limit + 1

        last_batch_of_groups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID, limit=limit, offset=offset)
        eq_(len(last_batch_of_groups), total - offset)

        # Check empty results
        no_groups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID, offset=total)
        eq_(no_groups, [])

        # Check full results
        offset = 0
        all_groups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_PROD_ACCOUNT_ID, offset=offset, limit=total + 1000)
    ...
    def test_find_by_ids(self):
        base_adgroups = self.fb.api().adgroup().find_by_adaccount_id(FACEBOOK_TEST_ACCOUNT_ID, limit=25)
        ...
        adgroups = self.fb.api().adgroup().find_by_ids(test_adgroup_ids[:10])
        ...
        try:
            adgroups = self.fb.api().adgroup().find_by_ids([])
        except Exception, e:
