...
class TestAdGroupApi(ApiTest):
    ...
    def test_find_by_adaccount_id(self):
        # Check order
        with patch('pyfacebook.PyFacebook._json_response', **self.get_patch_kwargs(AdGroup, number=10)):
    ...
    def test_find_by_id(self):
        with patch('pyfacebook.PyFacebook._json_response', **self.get_patch_kwargs(AdGroup, number=2)):
            ...
            adgroup = adgroups[0]
        with patch('pyfacebook.PyFacebook._json_response', **self.get_patch_kwargs(AdGroup)):
    ...
    def test_find_by_ids(self):
        ...
        test_adgroup_ids = map(lambda x: x.id, base_adgroups)[:10]  # cool way of pulling a simple list of attributes from a list of more complex objects
        with patch('pyfacebook.PyFacebook._json_response',
                   **self.get_patch_kwargs(AdGroup, number=len(test_adgroup_ids), nest_by_id=True)):
            adgroups = self.adgroup_api.find_by_ids(test_adgroup_ids)
        ...
        with patch('pyfacebook.PyFacebook._json_response', **self.get_patch_kwargs(AdGroup, number=0, nest_by_id=True)):
            try:
